...
public class TrustRankAlgorithm {
    ...
    private final SeedVecInitializer initializer;

    /**
     * This constructor is used to initialize the adjacency matrix that is going to be passed to trust rank
     ...
     * @param maxIter
     * @param initializer The type of initializer that would be used to initialize the seed factor
     */
    public TrustRankAlgorithm(double[][] adjMatrix, double[] lossVector, SeedVecInitializer initializer, int maxIter) {
        if (adjMatrix == null) {
    ...
    private HashMap<Integer, Double> startRanking() {
        ...
        if (initializer.equals(SeedVecInitializer.WeightFactorBased)) {
            for (int i = 0; i < adjMatrix.length; i++) {
                if (assetLossVec[i] > 0) {
            ...
            HashMap<Integer, Double> score = new RankBasedLossAversion(adjMatrix, assetLossVec, 0.7f).getScores();
            score.forEach(initialSeedVector::replace);
        }
    ...
    public enum SeedVecInitializer {
        WeightFactorBased,
        LossAversionBased
    }
}
