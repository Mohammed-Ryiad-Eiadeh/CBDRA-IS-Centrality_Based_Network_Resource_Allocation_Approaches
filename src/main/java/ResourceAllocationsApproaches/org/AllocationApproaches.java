...
import GraphCentralityMethods.org.ABNR;
...
import GraphCentralityMethods.org.RankBasedLossAversion;
...
public class AllocationApproaches {
    ...
    public Defenders[][] defendersMatrix;
    ...
    public void callCentrality(LinkageType linkageType, Centrality centrality) {
        ...
        HashMap<Integer, Double> scores = switch (centrality) {
            ...
            case TrustRank ->        new TrustRankAlgorithm(AdjMatrix, AssetLossVector, TrustRankAlgorithm.SeedVecInitializer.WeightFactorBased, 100).getScores();
            case ABNR ->             new ABNR(AdjMatrix, AssetLossVector, 100).getScores();
            case LossAversionRank -> new RankBasedLossAversion(AdjMatrix, AssetLossVector, alpha).getScores();
        };
        ...
        switch (linkageType) {
            ...
            case In_Degree_Nodes_With_Behavioral_Distribution -> {
                // This code segment is referred to capture the connections among each asset and its neighbor nodes
                InDegreeNodes IDN = new InDegreeNodes(AdjMatrix, AssetLossVector);
                HashMap<Integer, ArrayList<Integer>> nodes = IDN.retrieveInDegreeNodes();
                ...
                for (Integer assetNode : nodes.keySet()) {
                    ArrayList<Integer> parents = nodes.get(assetNode);
                    double sumOfWeightsBasedProspectForCurrentAsset = 0;
                    for (Integer incoming : parents) {
                        sumOfWeightsBasedProspectForCurrentAsset += ProbabilityWeight(AdjMatrix[incoming - 1][assetNode - 1], alpha);
                    }
                    for (Integer nod : parents) {
                        Defenders edge = defendersMatrix[nod - 1][assetNode - 1];
                        double currentAssetCutOfTotalBudget = budget * scores.get(assetNode) * (ProbabilityWeight(AdjMatrix[nod - 1][assetNode - 1], alpha) / sumOfWeightsBasedProspectForCurrentAsset);
                        edge.setInvest_D(edge.addSpareInvestFor_D(currentAssetCutOfTotalBudget));
                    }
                }
            }
    ...
    public enum LinkageType {
        ...
        Adjacent_Nodes,
        Min_Cut_Edges,
        In_Degree_Nodes_With_Behavioral_Distribution
    }
    ...
    public double ProbabilityWeight(double edgeID, float alpha) {
        return Math.exp(-Math.pow(-Math.log(Math.exp(-edgeID)), alpha));
    }

    public Defenders[][] getDefendersMatrix() {
        return defendersMatrix;
    }
}
